...
let UserRepository = class UserRepository {
    ...
    async create(user) {
        ...
        return new user_entity_1.User(saved.name, saved.email, saved.password, saved.id);
    }
    async findByEmail(email) {
        const ormUser = await this.ormRepo.findOne({ where: { email } });
        if (!ormUser)
            return null;
        return new user_entity_1.User(ormUser.name, ormUser.email, ormUser.password, ormUser.id);
    }
